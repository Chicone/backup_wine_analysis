...
def plot_pinot_noir_special_coloring(
    ...
    exclude_us=False,
    density_plot=False,
    color_by_burgns=False,  # NEW
    color_by_state=False  # NEW
):
    """
    Plot a 2D or 3D scatter plot of embedded data with labeled points.
    """
    ...
    if color_by_origin:
        ...
        state_colors = {
            "Drouhin": (0.70, 0.10, 0.10),
            "Bouchard": (0.10, 0.50, 0.70),
            "Jadot": (0.10, 0.60, 0.20)
        }
        state_markers = {
            "Drouhin": "o",
            "Bouchard": "s",
        ...
        legend_handles = []  # collect legend entries here

        # --- Case 1: Burgundy North vs South ---
        if color_by_burgns:
            colors = {"North": (0.70, 0.20, 0.20), "South": (0.20, 0.45, 0.75)}
            for group, codes in {"North": burgundy_north, "South": burgundy_south}.items():
                mask = np.isin(winery_codes, list(codes))
                coords = embedding[mask]
                if coords.shape[0] == 0:
                    continue
                color = colors[group]
                if density_plot and not is_3d:
                    plot_gaussian_cloud_2d(ax, coords, color, marker='o', readable_label=f"Burgundy {group}")
                if is_3d:
                    ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2], alpha=0.9, s=80, color=color, marker='o')
                else:
                    ax.scatter(coords[:, 0], coords[:, 1], alpha=0.9, s=80, color=color, marker='o')

                legend_handles.append(Line2D([0], [0], color=color, marker='o',
                                             linestyle='', markersize=8, label=f"Burgundy {group}"))

        # --- Case 2: Estate-level (Drouhin, Bouchard, Jadot) ---
        elif color_by_state:
            from scipy.spatial.distance import pdist

            state_codes = set(sum(state_mapping.values(), []))
            non_state_codes = [c for c in unique_codes if c not in state_codes]

            estate_coords = {}  # coords including all codes
            estate_coords_noX = {}  # coords excluding X from Drouhin

            # Estates
            for state, codes in state_mapping.items():
                mask = np.isin(winery_codes, codes)
                coords = embedding[mask]
                if coords.shape[0] == 0:
                    continue
                estate_coords[state] = coords

                # Special case: Drouhin without X
                if state == "Drouhin":
                    mask_noX = np.isin(winery_codes, ["D", "R"])
                    coords_noX = embedding[mask_noX]
                    if coords_noX.shape[0] > 0:
                        estate_coords_noX[state] = coords_noX
                else:
                    estate_coords_noX[state] = coords  # keep same for others

                # Plotting
                color = state_colors[state]
                marker = state_markers[state]
                if density_plot and not is_3d:
                    plot_gaussian_cloud_2d(ax, coords, color, marker, readable_label=state)
                if is_3d:
                    ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2],
                               alpha=0.9, s=80, color=color, marker=marker)
                else:
                ...
                legend_handles.append(Line2D([0], [0], color=color, marker=marker,
                                             linestyle='', markersize=8, label=state))

            # Other wineries individually (same as before)
            rng = random.Random(42)
            non_state_colors = distinctipy.get_colors(len(non_state_codes), rng=rng)
            non_state_color_map = dict(zip(non_state_codes, non_state_colors))

            for i, code in enumerate(non_state_codes):
                mask = (winery_codes == code)
                coords = embedding[mask]
                if coords.shape[0] == 0:
                    continue
                color = non_state_color_map[code]
                marker = markers[i % len(markers)]
                readable_label = label_dict.get(code, code)
                ...
                legend_handles.append(Line2D([0], [0], color=color, marker=marker,
                                             linestyle='', markersize=8, label=readable_label))

            # === ICD COMPUTATION ===
            def compute_within_icd(estate_coords):
                out = {}
                for estate, coords in estate_coords.items():
                    out[estate] = pdist(coords).mean() if coords.shape[0] > 1 else np.nan
                return out

            def compute_between_icd(estate_coords):
                vals = []
                estate_list = list(estate_coords.keys())
                for i in range(len(estate_list)):
                    for j in range(i + 1, len(estate_list)):
                        e1, e2 = estate_list[i], estate_list[j]
                        c1, c2 = estate_coords[e1], estate_coords[e2]
                        dists = np.linalg.norm(c1[:, None, :] - c2[None, :, :], axis=-1).ravel()
                        vals.append(dists.mean())
                return np.mean(vals), np.std(vals)

            print("\n=== Intra- vs Inter-estate ICD (with X) ===")
            within_full = compute_within_icd(estate_coords)
            for estate, val in within_full.items():
                print(f"Within {estate}: {val:.3f}")
            mean_between, std_between = compute_between_icd(estate_coords)
            print(f"Average between-estates ICD: {mean_between:.3f} ± {std_between:.3f}")

            print("\n=== Intra- vs Inter-estate ICD (without Drouhin X) ===")
            within_noX = compute_within_icd(estate_coords_noX)
            for estate, val in within_noX.items():
                print(f"Within {estate}: {val:.3f}")
            mean_between_noX, std_between_noX = compute_between_icd(estate_coords_noX)
            print(f"Average between-estates ICD: {mean_between_noX:.3f} ± {std_between_noX:.3f}")
            # state_codes = set(sum(state_mapping.values(), []))
            # non_state_codes = [c for c in unique_codes if c not in state_codes]
            #
            # # Estates
            ...
            #         plot_gaussian_cloud_2d(ax, coords, color, marker, readable_label=state)
            #     if is_3d:
            #         ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2], alpha=0.9, s=80, color=color, marker=marker)
            #     else:
            #         ax.scatter(coords[:, 0], coords[:, 1], alpha=0.9, s=80, color=color, marker=marker)
            #
            #     legend_handles.append(Line2D([0], [0], color=color, marker=marker,
            #                                  linestyle='', markersize=8, label=state))
            #
            # # Other wineries individually
            # rng = random.Random(42)
            # non_state_colors = distinctipy.get_colors(len(non_state_codes), rng=rng)
            # non_state_color_map = dict(zip(non_state_codes, non_state_colors))
            #
        ...
        # --- Case 3: Default cru-level plotting ---
        else:
            rng = random.Random(42)
            colors = distinctipy.get_colors(len(unique_codes), rng=rng)
        ...
        if legend_handles:
            ...
            legend = ax.legend(handles=legend_handles, labels=labels,
                               fontsize='large', loc='best', frameon=True)
            ax.add_artist(legend)  # 👈 force attach legend to current axis
        else:
            print("⚠️ No legend entries were generated!")
    ...
    #             if is_3d:
    #                 ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2],
    #                            label=f"Burgundy {group}", alpha=0.9, s=80, color=color, marker='o')
    #             else:
    #                 ax.scatter(coords[:, 0], coords[:, 1],
    #                            label=f"Burgundy {group}", alpha=0.9, s=80, color=color, marker='o')
    #
    ...
    #             if is_3d:
    #                 ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2],
    #                            label=state, alpha=0.9, s=80, color=color, marker=marker)
    #             else:
    #                 ax.scatter(coords[:, 0], coords[:, 1],
    #                            label=state, alpha=0.9, s=80, color=color, marker=marker)
    #
    ...
    #     burgundy_south = {'P', 'R', 'Z'}
    #
    #     # Two fixed colors for N/S Burgundy
    #     burg_n_color = (0.70, 0.20, 0.20)  # deep red
    #     burg_s_color = (0.20, 0.45, 0.75)  # blue
    #
    #     # Build colors for non-Burgundy wineries (distinct)
    #     non_burgundy = [c for c in unique_codes if c not in burgundy_north | burgundy_south]
    #     rng = random.Random(42)
    #     non_b_colors = distinctipy.get_colors(len(non_burgundy), rng=rng)
    #     non_b_color_map = dict(zip(non_burgundy, non_b_colors))
    #
    #     # Fixed marker order, consistent across all wineries
    #     markers = ['o', 's', '^', 'v', 'D', 'X', '*', 'P', 'h', '8', '<', '>', 'p', 'H', 'd', '1']
    #
    #     # --- 1) Plot Burgundy groups first (as two aggregates) ---
    #     if color_by_burgns:
    #         # North
    #         bn_mask = np.isin(winery_codes, list(burgundy_north))
    #         if np.any(bn_mask):
    #             bn_coords = embedding[bn_mask]
    #             if density_plot and not is_3d:
    #                 plot_gaussian_cloud_2d(ax, bn_coords, burg_n_color, marker='o', readable_label="Burgundy North")
    #             ax.scatter(
    #                 bn_coords[:, 0], bn_coords[:, 1],
    #                 color=burg_n_color, marker='o', s=80, alpha=0.9, label="Burgundy North"
    #             )
    #
    #         # South
    #         bs_mask = np.isin(winery_codes, list(burgundy_south))
    #         if np.any(bs_mask):
    #             bs_coords = embedding[bs_mask]
    #             if density_plot and not is_3d:
    #                 plot_gaussian_cloud_2d(ax, bs_coords, burg_s_color, marker='o', readable_label="Burgundy South")
    #             ax.scatter(
    #                 bs_coords[:, 0], bs_coords[:, 1],
    #                 color=burg_s_color, marker='o', s=80, alpha=0.9, label="Burgundy South"
    #             )
    #
    #     # --- 2) Plot each non-Burgundy winery with its own color and legend label ---
    #     for code in non_burgundy:
    #         mask = (labels == code)
    #         if not np.any(mask):
    #             continue
    #         coords = embedding[mask]
    #         color = non_b_color_map[code]
    #
    #         # Use marker index from full unique_codes list (ensures consistency)
    #         idx = unique_codes.index(code)
    ...
    #         if density_plot and not is_3d:
    #             plot_gaussian_cloud_2d(ax, coords, color, marker, readable_label)
    #             ax.scatter(coords[:, 0], coords[:, 1], color=color, marker=marker,
    #                        s=80, alpha=0.9, label=readable_label, zorder=999)
    #         else:
    #             if is_3d:
    #                 ax.scatter(coords[:, 0], coords[:, 1], coords[:, 2],
    ...
    # elif color_by_state:
    #     if raw_sample_labels is None:
    #         raise ValueError("raw_sample_labels must be provided when color_by_state=True")
    #
    #     winery_codes = np.array([s[0] for s in raw_sample_labels])
    #     unique_codes = sorted(set(winery_codes))
    #     is_3d = (embedding.shape[1] == 3)
    #
    #     # Define the state groupings
    #     state_mapping = {
    #         "Drouhin": ["D", "R", "X"],   # Burgundy + US
    #         "Bouchard": ["E", "Q"],
    #         "Jadot": ["P", "Z"]
    #     }
    #
    #     state_colors = {
    #         "Drouhin": (0.70, 0.10, 0.10),   # deep red
    #         "Bouchard": (0.10, 0.50, 0.70),  # blue
    #         "Jadot": (0.10, 0.60, 0.20)      # green
    #     }
    #
    #     state_markers = {
    #         "Drouhin": "o",
    ...
    #     }
    #
    #     # Track codes that belong to collapsed states
    #     state_codes = set(sum(state_mapping.values(), []))
    #     non_state_codes = [c for c in unique_codes if c not in state_codes]
    #
    #     # Distinct colors for non-state codes
    #     rng = random.Random(42)
    #     non_state_colors = distinctipy.get_colors(len(non_state_codes), rng=rng)
    #     non_state_color_map = dict(zip(non_state_codes, non_state_colors))
    #
    #     markers = ['o', 's', '^', 'v', 'D', 'X', '*', 'P', 'h', '8', '<', '>', 'p', 'H', 'd', '1']
    #
    #     # --- 1) Plot the collapsed states ---
    #     for state, codes in state_mapping.items():
    #         mask = np.isin(labels, codes)  # use labels, not winery_codes
    #         coords = embedding[mask]
    #         if coords.shape[0] == 0:
    #             continue
    #         color = state_colors[state]
    ...
    #                        label=state, alpha=0.9, s=80, color=color, marker=marker)
    #
    #     # --- 2) Plot non-state wineries individually ---
    #     for i, code in enumerate(non_state_codes):
    #         mask = (labels == code)  # select by raw_sample_labels first letter
    #         coords = embedding[mask]
    #         if coords.shape[0] == 0:
    #             continue
    #         color = non_state_color_map[code]
    ...
    if color_by_winery or color_by_origin:
        ...
        # Include Burgundy groups first (only if plotted)
        if color_by_winery and color_by_burgns:
            if "Burgundy North" in label_to_handle:
                ordered_handles.append(label_to_handle["Burgundy North"])
                ordered_labels.append("Burgundy North")
            if "Burgundy South" in label_to_handle:
                ordered_handles.append(label_to_handle["Burgundy South"])
                ordered_labels.append("Burgundy South")

        # Then add non-Burgundy wineries in label_dict order
        for key in label_dict:
            readable = label_dict[key]
            # Skip Burgundy codes if we’re collapsing them
            if color_by_winery and color_by_burgns and key in (burgundy_north | burgundy_south):
                continue
            if readable in label_to_handle:
                ordered_handles.append(label_to_handle[readable])
                ordered_labels.append(readable)
